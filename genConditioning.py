...
#        self.initial_fc = nn.Linear(encoded_feature_dim, embedding_dim)
        ...
        self.F0_FC = nn.Linear(self.embedding_dim, 184)
        # Generates h0
    ...
    def forward(self, sentence, imgs):
        ...
        resized = self.F0_FC(sentence.squeeze(0))
        
        mu = resized[:,:int(resized.shape[1]/2)]
        sig = resized[:,int(resized.shape[1]/2):]
        sig = torch.mul(sig, torch.randn_like(sig, device=self.device))
        
        c0 = torch.cat([mu+sig, torch.randn((mu.shape[0], 100), device=self.device)], dim=1)
        h0 = self.F0(c0.view(imgs.shape[0], 3, 8, 8))
        gen_img = self.G0(h0)
