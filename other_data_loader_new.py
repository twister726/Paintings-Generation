...
#                     print("Weirdooo" + l)
    ...
    def __getitem__(self, index):
        ...
        # Mislabeled caption
        wrong_caption = self.cap2cap[random.choice(self.ids)]
        ...
        tokens.insert(0, '<start>')
        tokens.append('<end>')
        wrong_length = len(tokens)
        if wrong_length < self.maxCapLen:
            tokens.extend([pad]*(self.maxCapLen - wrong_length))
        
        wrong_caption = [vocab(token) for token in tokens[:self.maxCapLen]]
        wrong_target = torch.Tensor(wrong_caption)
        
#         print('length: ', length)
#         print('wrong length: ', wrong_length)

        return image, target, wrong_target, length, wrong_length
...
#         print(d.shape)
    ...
    images, captions, wrong_captions, lengths, wrong_lengths = zip(*data)
    
#     print('captions: ', captions)
#     print('wrong captions: ', wrong_captions)
#     print('len data[0]: ', len(data[0]))
#     print('lengths: ', lengths)
#     print('wrong_lengths: ', wrong_lengths)
#     print('len data[0][1]: ', len(data[0][1]))
#     print('len data[0][2]: ', len(data[0][2]))
    ...
    wrong_targets = torch.zeros(len(wrong_captions), len(data[0][2])).long()
    for i, cap in enumerate(wrong_captions):
        wrong_targets[i, :wrong_lengths[i]] = cap[:wrong_lengths[i]]
    
    return images, targets, wrong_targets, lengths, wrong_lengths
